/**
 * Audit Trail API - Single Decision Detail (Edge Runtime)
 * 
 * GET /api/audit/:id
 * Returns full decision record with context and reasoning
 */

export const config = {
  runtime: 'edge',
};

// Demo decision detail for showcase
const DEMO_DECISIONS: Record<string, any> = {
  '1706918400000-rf2k8m': {
    id: '1706918400000-rf2k8m',
    timestamp: 1706918400000,
    time: '2026-02-03T00:00:00.000Z',
    type: 'rebalance',
    confidence: 0.82,
    executed: true,
    txIds: ['5cGz9XnBhPvHmTzYkQrNpqJmvwRsK3kLxMbYdEfA7hJc'],
    reasoning: `📊 **Risk-Adjusted Analysis**
Found 12 opportunities within risk tolerance

🏆 **Top Recommendation:** USDC on kamino
   • Raw APY: 11.50%
   • Risk-adjusted APY: 9.43%
   • Risk Score: 25/100
   • Sharpe Ratio: 2.31
   • TVL: $89.2M

✅ Audited by OtterSec, Halborn
✅ Insurance fund available
✅ Battle-tested (2+ years)

📈 **Portfolio Comparison**
   • Current APY: 7.20%
   • Projected APY: 11.50%
   • Risk-adjusted improvement: 2.23%

🔄 **Decision: REBALANCE**
Moving to better risk-adjusted opportunities
Risk change: decreased`,
    actions: [
      {
        type: 'withdraw',
        from: { protocol: 'marinade', asset: 'mSOL', amount: 10.5 },
        to: { protocol: 'kamino', asset: 'USDC', amount: 1890 },
        expectedApyGain: 4.3,
      },
    ],
    riskAnalysis: {
      currentRiskScore: 28,
      proposedRiskScore: 25,
      riskChange: 'decreased',
      topOpportunity: {
        protocol: 'kamino',
        asset: 'USDC',
        rawApy: 11.5,
        adjustedApy: 9.43,
        sharpeRatio: 2.31,
        riskScore: 25,
        warnings: [],
        positives: ['Audited by OtterSec, Halborn', 'Insurance fund available', 'Strong TVL ($89.2M)'],
      },
    },
    context: {
      portfolioSnapshot: {
        totalValue: 1890,
        weightedApy: 7.2,
        positions: [
          { protocol: 'marinade', asset: 'mSOL', amount: 10.5, valueUsd: 1890, currentApy: 7.2 },
        ],
      },
      strategyConfig: {
        name: 'Balanced',
        riskTolerance: 'medium',
        rebalanceThreshold: 1.0,
        maxProtocolConcentration: 0.5,
        maxSlippage: 0.01,
      },
      marketConditions: {
        solPrice: 180,
        avgApy: 8.5,
      },
      topYieldsAvailable: [
        { protocol: 'kamino', asset: 'USDC', rawApy: 11.5, adjustedApy: 9.43, riskScore: 25 },
        { protocol: 'drift', asset: 'USDC', rawApy: 10.2, adjustedApy: 7.85, riskScore: 30 },
        { protocol: 'jito', asset: 'JitoSOL', rawApy: 7.8, adjustedApy: 6.94, riskScore: 20 },
        { protocol: 'marinade', asset: 'mSOL', rawApy: 7.2, adjustedApy: 5.88, riskScore: 28 },
      ],
    },
    meta: {
      date: '2026-02-03',
      decisionType: 'rebalance',
      protocols: ['marinade', 'kamino'],
      assets: ['mSOL', 'USDC'],
    },
  },
  '1706914800000-k4m9q2': {
    id: '1706914800000-k4m9q2',
    timestamp: 1706914800000,
    time: '2026-02-02T23:00:00.000Z',
    type: 'hold',
    confidence: 0.91,
    executed: false,
    reasoning: `📊 **Risk-Adjusted Analysis**
Found 8 opportunities within risk tolerance

Current position: mSOL on Marinade
   • Current APY: 7.20%
   • Risk Score: 28/100

Best alternative: JitoSOL staking
   • Raw APY: 7.8%
   • Risk-adjusted APY: 6.9%
   • Potential improvement: 0.3%

⏸️ **Decision: HOLD**
Improvement of 0.3% is below rebalance threshold of 1%
Transaction costs would exceed benefit
Holding current position`,
    actions: [],
    riskAnalysis: {
      currentRiskScore: 28,
      proposedRiskScore: 28,
      riskChange: 'unchanged',
    },
    context: {
      portfolioSnapshot: {
        totalValue: 1890,
        weightedApy: 7.2,
        positions: [
          { protocol: 'marinade', asset: 'mSOL', amount: 10.5, valueUsd: 1890, currentApy: 7.2 },
        ],
      },
      strategyConfig: {
        name: 'Balanced',
        riskTolerance: 'medium',
        rebalanceThreshold: 1.0,
      },
    },
    meta: {
      date: '2026-02-02',
      decisionType: 'hold',
      protocols: ['marinade'],
      assets: ['mSOL'],
    },
  },
};

export default async function handler(req: Request): Promise<Response> {
  const url = new URL(req.url);
  const pathParts = url.pathname.split('/');
  const id = pathParts[pathParts.length - 1];

  // Check if requesting replay context
  const isReplay = pathParts.includes('replay');

  const decision = DEMO_DECISIONS[id];

  if (!decision) {
    // Return a helpful response for demo
    return new Response(JSON.stringify({
      success: false,
      error: `Decision not found: ${id}`,
      availableIds: Object.keys(DEMO_DECISIONS),
      tip: 'Try one of the available IDs, e.g., /api/audit/1706918400000-rf2k8m',
    }, null, 2), {
      status: 404,
      headers: {
        'Content-Type': 'application/json',
        'Access-Control-Allow-Origin': '*',
      },
    });
  }

  if (isReplay) {
    // Return replay context with previous decisions
    return new Response(JSON.stringify({
      success: true,
      description: 'Full decision replay context - see exactly what the agent saw',
      decision: {
        id: decision.id,
        timestamp: decision.timestamp,
        time: decision.time,
        type: decision.type,
        confidence: decision.confidence,
        executed: decision.executed,
        reasoning: decision.reasoning,
        actions: decision.actions,
        riskAnalysis: decision.riskAnalysis,
      },
      fullContext: decision.context,
      previousDecisions: [
        {
          id: '1706914800000-k4m9q2',
          timestamp: 1706914800000,
          time: '2026-02-02T23:00:00.000Z',
          type: 'hold',
          confidence: 0.91,
          executed: false,
        },
        {
          id: '1706911200000-p8n3x7',
          timestamp: 1706911200000,
          time: '2026-02-02T22:00:00.000Z',
          type: 'enter',
          confidence: 0.78,
          executed: true,
        },
      ],
      summaryMarkdown: `# Decision Replay Summary

**Decision ID:** ${decision.id}
**Time:** ${decision.time}
**Type:** ${decision.type.toUpperCase()}
**Confidence:** ${(decision.confidence * 100).toFixed(0)}%
**Executed:** ${decision.executed ? '✅ Yes' : '❌ No'}

## Portfolio at Decision Time
- Total Value: $${decision.context.portfolioSnapshot.totalValue.toFixed(2)}
- Weighted APY: ${decision.context.portfolioSnapshot.weightedApy.toFixed(2)}%
- Positions: ${decision.context.portfolioSnapshot.positions.length}

## Risk Analysis
${decision.riskAnalysis ? `- Current Risk Score: ${decision.riskAnalysis.currentRiskScore}/100
- Proposed Risk Score: ${decision.riskAnalysis.proposedRiskScore}/100
- Risk Change: ${decision.riskAnalysis.riskChange}` : 'N/A'}

## Full Reasoning
\`\`\`
${decision.reasoning}
\`\`\`
`,
    }, null, 2), {
      status: 200,
      headers: {
        'Content-Type': 'application/json',
        'Access-Control-Allow-Origin': '*',
      },
    });
  }

  // Return standard decision detail
  return new Response(JSON.stringify({
    success: true,
    ...decision,
  }, null, 2), {
    status: 200,
    headers: {
      'Content-Type': 'application/json',
      'Access-Control-Allow-Origin': '*',
    },
  });
}
